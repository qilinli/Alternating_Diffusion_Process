    ...
    if strcmpi(options.Metric,'Euclidean')
        ...
        for i = 1:ceil(nSmp/nBlock)
            if i == ceil(nSmp/nBlock)
                ...
                dump = dump(:,1:options.k+1);
                %dump = exp(-dump/(2*options.t^2));
                %%%Revised by Jun Wang (jwang@ee.columbia.edu) Oct. 18,
                %%%2007
                if strcmpi(options.Adaptive,'Yes')
                    dump_temp=zeros(size(dump,1),size(dump,2));
                    for i_wj=1:size(dump,1)
                        if i_wj==353
                            ttt=1;
                        end
                        for j_wj=2:size(dump,2)
                            p1_ind=i_wj;
                            p2_ind=idx(i_wj,j_wj);
                            sigma=(0.5*mean(sqrt(dump(p1_ind,:)))+0.5*mean(sqrt(dump(p2_ind,:))));
                            %sigma=max([max(dump(p1_ind,:)) max(dump(p2_ind,:))]);
                            if dump(i_wj,j_wj)==0
                                dump_temp(i_wj,j_wj)=1;
                            else                               
                                dump_temp(i_wj,j_wj)=exp(-dump(i_wj,j_wj)/(2*sigma^2));
                            end
                        end
                    end
                    dump=dump_temp;
